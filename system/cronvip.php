...
$vip = mysqli_query($GLOBALS["___BMN_2312"], "SELECT * FROM idvip ORDER BY RAND() LIMIT 0,5");
if($vip){
while($vipi = mysqli_fetch_array($vip))
  {      
    $idvip = $vipi['id_user'];
    $limitlike = $vipi['limitlike']  + $vipi['liketrungbinh'];
    $limitpost = $vipi['limitpost'];    
    $camxuc = $vipi['camxuc'];
    $checkgioihanstt = mysqli_fetch_array(mysqli_query($GLOBALS["___BMN_2312"], "SELECT COUNT(*) FROM `log_gioihan` WHERE `id_user` ='" . $idvip. "'"),  0);
    if($checkgioihanstt <= $limitpost)
  {
$result = mysqli_query($GLOBALS["___BMN_2312"], "SELECT * FROM token ORDER BY RAND() LIMIT 0,500");
if($result){
...
$ghi_log = mysqli_fetch_array(mysqli_query($GLOBALS["___BMN_2312"], "SELECT COUNT(*) FROM `log_gioihan` WHERE `id_stt` ='" . $puaruid . "'"),  0);    
if($ghi_log == 0){    
mysqli_query($GLOBALS["___BMN_2312"], "INSERT INTO log_gioihan SET `id_user` = '" . $idvip . "',`id_stt` = '" . $puaruid . "'");
}
