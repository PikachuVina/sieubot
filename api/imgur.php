...
if (!isset($_POST['photoimg']))
  die(header('location: upanh.php/?er=1'));
if (!isset($_FILES['file']['name']) or $_FILES['file']['name'] == null)
  die(header('location: upanh.php/?er=7'));
// Kiểm tra size
if ($_FILES['file']['size'] > 20000000)
  die(header('location: upanh.php/?er=2'));
//Kiểm tra nếu là ảnh
if (!getimagesize($_FILES['file']['tmp_name']))
  die(header('location: upanh.php/?er=3'));
// Kiểm tra phân giải
list($width, $height, $type, $attr) = getimagesize($_FILES['file']['tmp_name']);
if ($width > 30000 || $height > 30000)
  die(header('location: upanh.php/?er=4'));
...
if($checkimage == false){
     die(header('location: upanh.php/?er=5'));
}
...
if ($reply['success'] == 1) {
  die(header('location: upanh.php/?link=' . base64_encode($reply['data']['link'])));
}
else {
  die(header('location: upanh.php/?er=6'));
}
